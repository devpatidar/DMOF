...
function dmof_meta_boxes( $meta_id , $post_id = ''){
...
function dmb_get_attachment_data( $meta_id , $post_id = null , $output = 'id' , $size = 'full'){
	...
	// get other parameter
	if ($output == 'img') { return wp_get_attachment_image( $post_meta_val, $size ); }
	if ($output == 'url') { return wp_get_attachment_url( $post_meta_val , $size ); }
...
function dmb_get_multiple_images($meta_id , $post_id = null , $size = 'full' , $output = 'img') {
	...
	$dmb_get_img_id = dmof_meta_boxes($meta_id,$post_id);
	$dmb_output_img = '';
